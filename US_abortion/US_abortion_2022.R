...
map_data <- state_map |> mutate(region=str_to_title(region))|> left_join(prohibt, by=c("region"="state")) |> mutate(pattern=case_when(is.na(status)~"weave", TRUE~pattern))
